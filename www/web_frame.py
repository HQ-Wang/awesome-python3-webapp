...
"""
@file: web_frame.py
@time: 2016/4/25 14:19
"""
__author__ = 'Hongqing Wang'

import asyncio
import os
# python中的自省模块，类似完成java一样的反射功能。具有类型判断、获取元信息等功能，具体查看官方文档
...
# ----------------使用inspect模块中的signature方法来获取函数的参数，实现一些复杂功能
# 关于inspect.Parameter 的kind类型有5种：
# POSITIONAL_ONLY           只能是位置参数
# POSITIONAL_OR_KEYWORD     可以使位置参数也可以是关键字参数
...
class RequestHandler(object):   # 初始化一个请求处理类
    ...
    # __call__方法的代码逻辑：
    # 1，定义kw对象，用于保存参数
    # 2，判断request对象是否存在参数，如果存在则根据市POST还是GET方法将参数内容保存到kw
    ...
    def __call__(self, request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            ...
            # POST提交请求的类型（通过content_type可以指定）可以参考：
            # http://kaimingwan.com/post/python/postchang-jian-qing-qiu-fang-shi-qian-xi
        ...
        # kw不为空时，还要判断下是可变参数还是命名关键字参数，如果是命名关键字参数，则需要remove all unamed kw，原因不明？
        else:
            # 如果从Request对象中获取到参数了
            # 当没有可变参数，有命名关键字参数时候，kw指向命名关键字参数的得荣
...
def add_static(app):
    ...
    app.router.add_static('/static/', path)     # app是aiohttp库里面的对象，通过router.add_router方法可以指定处理函数。本节代码自己实现了add_router.更多可查看aiohttp的库文档：http://aiohttp.readthedocs.org/en/stable/web.html
...
def add_route(app, fn):
    ...
    # 判断fn是不是协程（即@asyncio.coroutine修饰的）并且判断是不是fn是不是一个生成器（generator function
...
def add_routes(app, module_name):
    ...
    if n == (-1):
        ...
        # 上面两行是廖老师源代码，但是把传入参数module_name的值改为handlers.py的话走这里是报错的，所以改成下面这样
    ...
    for attr in dir(mod):
        ...
        if callable(fn):
            ...
            path = getattr(fn, '__route__', None)
            if method and path:
                # 如果都有，说明我们定义的处理方法，加到app对象里处理route中
                add_route(app, fn)
