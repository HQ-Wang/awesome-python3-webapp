...
def init_jinja2(app, **kw):
	...
	options = dict(
		# 是否转义设置为True，就是在渲染模板时自动把变量中的<>&等字符转换为&lt;&gt;&amp
		...
		# jinja2会在使用template时检查模板文件的状态，如果模板有修改，则重新加载模板。如果对性能要求较高，可将此值设为False
	...
	# Environment是jinja2中的一个核心类，它的实例用来保存设置、全局对象，以及从本地文件系统或其他位置加载模板。
	...
	def auth(request):
		...
		if cookie_str:
			...
			if user:
				...
				# user 存在则绑定到request上，说明当前用户是合法的
...
# 那么结果处理的情况就是：
#		由handler构造出要返回的具体对象
#		然后在这个返回的对象上加上'__method__'和'__route__'属性，以标识符识别这个对象并使接下来的程序容易处理
#		RequestHandler目前就是从URL函数中分析其需要接收的参数，从request中获取必要的参数，调用URL函数，然后把结果返回给response_factory
#		response_factory在拿到经过处理后的对象，经过一些列对象类型和格式的判断，构造出正确web.Response对象，以正确的方式返回给客户端
# 在这个过程中，我们只用关系我们的handler的处理就好了，其他的都走统一的通道，如果需要差异化处理，就在通道中选择适合的地方添加处理代码
...
def response_factory(app, handler):
	...
	def response(request):
		...
		if isinstance(r, tuple) and len(r) == 2:
			t, m = r
			# 如果tuple的第一个元素是int类型且在100到600之间，这里应该认为tweihttp状态码，m为错误描述
			# 或者是服务端自己定义的错误码+描述
