...
def create_pool(loop, **kw):
    ...
    __pool = yield from aiomysql.create_pool(
        host=kw.get('host', 'localhost'),       # host固定为localhost
        port=kw.get('port', 3306),              # 默认定义mysql的port固定为3306
        user=kw['user'],                        # user取传入的参数key为user的值
        password=kw['password'],                # password取传入的参数key为password的值
        db=kw['db'],                            # 数据库名字，做ORM测试的使用请使用db=kw['db']
        charset=kw.get('charset', 'utf8'),      # 默认数据库字符集编码为utf8
...
# ============================================SQL处理函数区==============================================
# select和execute方法是实现其他Model类中SQL语句都经常使用的方法，原本是全局函数，这里作为静态函数处理
...
def select(sql, args, size=None):
    ...
    # with局域用法可以参考：http://kaimingwan.com/post/python/pythonzhong-de-withyu-ju-shi-yong

    # 异步等待连接池对象返回可以连接线程，with语句则封装了清理（关闭conn）和处理异常的工作
    with (yield from __pool) as conn:
        # 等待连接对象返回DictCursor可以通过dict的方式获取数据可对象，需要通过游标对象执行SQL
...
def execute(sql, args, autocommit=True):
    ...
    with (yield from __pool) as conn:
        ...
        try:
            ...
            raise e     # raise 不带参数，则把此处的错误往上抛；为了方便理解还是建议加e
        return affected
...
class ModelMetaclass(type):
    ...
    # 1，任何继承自Model的类（比如User），会自动通过ModelMetacalss扫描映射关系
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for k, v in attrs.item():
            if isinstance(v, Field):    # 如果是Field类型的则加入mappings对象
                ...
                if v.primary_key:   # 如果v是主键即primary_key=True，尝试把其赋值给primaryKey属性
                    if primaryKey:  # 如果primaryKey属性已经不为空，说明已经有主键了，则抛出错误，因为只能一个主键
                        raise RuntimeError('Duplicate primary key for field: %s' % k)
                    primaryKey = k  # 如果主键还没被赋过值，则直接复制
        ...
        for k in mappings.keys():   # 清除mappings，防止实例属性覆盖类的同名属性，造成运行时错误
            # attrs中对应的属性需要删除。坐着指的是attrs的属性和mappings中的属性发生冲突，具体原因自行体会
            attrs.pop(k)
